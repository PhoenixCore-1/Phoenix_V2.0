...
class CoreApi:
    ...
    def get_current_identity(self, *, request_id: str, session_id, organisation_id=None) -> ApiResponse:
        ...
        return ApiResponse(
            data={"id": str(identity.id), "type": identity.identity_type, "status": identity.status},
    ...
    def _audit(self, context, *, action: str, target_type: str, target_id: UUID | None = None) -> None:
        self.core_service.audit_service.record(
            AuditEvent.create(
                action=action,
    ...
    def company_create_user(self, context, *, username: str, display_name: str, password: str) -> ApiResponse:
        ...
        try:
            ...
            # User creation is a Core operation; do not silently create a second tenant authority.
            # The caller receives the authoritative Core error if membership creation fails.
            raise
    ...
    def company_update_user(self, context, user_id: UUID, *, username: str | None = None, display_name: str | None = None) -> ApiResponse:
        ...
        membership = self.core_service.list_memberships(context.organisation_id)
        if not any(item.identity_id == user.identity_id for item in membership):
