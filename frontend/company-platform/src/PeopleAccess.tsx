...
export function PeopleAccess() {
  ...
  const load = async () => { setLoading(true); setNotice(''); try { await Promise.all([loadUsers(), loadRoles()]) } catch (error) { setNotice(error instanceof Error ? error.message : 'Unable to load People & Access.') } finally { setLoading(false) } }
  useEffect(() => { void load() }, [])
  ...
  return <div className="people-page">
    ...
    {tab === 'people' ? <>
      ...
      <div className="access-summary"><div><span>ROLES</span><strong>{roles.length}</strong></div><div><span>ACTIVE</span><strong>{roles.filter((r) => r.status === 'ACTIVE').length}</strong></div><div><span>SCOPE</span><strong>Tenant</strong></div><div><span>AUTHORITY</span><strong>Core controlled</strong></div></div>
