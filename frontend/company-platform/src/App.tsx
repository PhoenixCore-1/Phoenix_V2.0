...
function PeopleAccess() {
  ...
  const loadUsers = async () => {
    ...
    try { const response = await fetch('/api/v1/company/users', { credentials: 'include' }); if (!response.ok) throw new Error('Core API unavailable'); const body = await response.json() as { data?: { items?: UserRow[] } }; setUsers(body.data?.items ?? []) }
  ...
  return <div className="people-page">
    <div className="people-toolbar"><div><span className="section-label">PEOPLE & ACCESS</span><h3 style={{ margin: '5px 0' }}>Company users</h3></div><button className="primary-action" onClick={() => setNotice('User provisioning is controlled by the Phoenix Core company-user boundary.')}>+ Add user</button></div>
