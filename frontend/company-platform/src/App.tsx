...
function PeopleAccess() {
  ...
  return <div className="people-page">
    <div className="people-toolbar"><div className="people-tabs">{(['Users', 'Memberships', 'Roles'] as const).map((item) => <button key={item} className={`people-tab ${tab === item ? 'active' : ''}`} onClick={() => setTab(item)}>{item}</button>)}</div><button className="primary-action" onClick={() => setNotice('User provisioning will use the Core company-user mutation boundary once enabled.')}>+ Add user</button></div>
    <div className="access-summary"><div><span>ACTIVE USERS</span><strong>{users.filter((u) => u.membership_status === 'ACTIVE').length}</strong></div><div><span>SUSPENDED</span><strong>{users.filter((u) => u.membership_status === 'SUSPENDED').length}</strong></div><div><span>ROLES</span><strong>—</strong></div><div><span>ACCESS MODEL</span><strong>Core controlled</strong></div></div>
    {tab === 'Users' ? <>
      ...
      <div className="people-table-wrap"><table className="people-table"><thead><tr><th>PERSON</th><th>USERNAME</th><th>ACCESS</th><th>USER STATUS</th><th></th></tr></thead><tbody>
