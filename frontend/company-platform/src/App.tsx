...
const navigation: NavItem[] = [
  ...
  { label: 'Data Visibility', icon: 'visibility' }, { label: 'Activity', icon: 'activity' }, { label: 'Reports', icon: 'reports' }, { label: 'Company Settings', icon: 'settings' },
]
const management = [
...
function PeopleAccess() {
  const [tab, setTab] = useState<'Users' | 'Memberships' | 'Roles'>('Users')
  ...
  const loadUsers = async () => {
    ...
    try {
      const response = await fetch('/api/v1/company/users', { credentials: 'include' })
      if (!response.ok) throw new Error('Core API unavailable')
      const body = await response.json() as { data?: { items?: UserRow[] } }
      setUsers(body.data?.items ?? [])
    } catch { setNotice('Connect this workspace to an authenticated Phoenix Core session to load company users.') }
  ...
  const filtered = useMemo(() => users.filter((user) => {
    const matchesQuery = `${user.display_name} ${user.username}`.toLowerCase().includes(query.toLowerCase())
    const matchesStatus = status === 'ALL' || user.membership_status === status
    return matchesQuery && matchesStatus
  }), [users, query, status])

  return <div className="people-page">
    <div className="people-toolbar">
      <div className="people-tabs">{(['Users', 'Memberships', 'Roles'] as const).map((item) => <button key={item} className={`people-tab ${tab === item ? 'active' : ''}`} onClick={() => setTab(item)}>{item}</button>)}</div>
      <button className="primary-action" onClick={() => setNotice('User provisioning will use the Core company-user mutation boundary once enabled.')}>+ Add user</button>
    </div>

    <div className="access-summary">
      <div><span>ACTIVE USERS</span><strong>{users.filter((u) => u.membership_status === 'ACTIVE').length}</strong></div>
      <div><span>SUSPENDED</span><strong>{users.filter((u) => u.membership_status === 'SUSPENDED').length}</strong></div>
      <div><span>ROLES</span><strong>—</strong></div>
      <div><span>ACCESS MODEL</span><strong>Core controlled</strong></div>
    </div>

    {tab === 'Users' ? <>
      <div className="people-controls"><label className="search-field"><Icon name="search" size={15} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search people..." /></label><select value={status} onChange={(e) => setStatus(e.target.value)}><option value="ALL">All statuses</option><option value="ACTIVE">Active</option><option value="SUSPENDED">Suspended</option><option value="REMOVED">Removed</option></select><button className="refresh-button" onClick={() => void loadUsers()}>Refresh</button></div>
      <div className="people-table-wrap">
        <table className="people-table"><thead><tr><th>PERSON</th><th>USERNAME</th><th>ACCESS</th><th>USER STATUS</th><th></th></tr></thead><tbody>
          {loading ? <tr><td colSpan={5} className="table-empty">Loading company users…</td></tr> : filtered.length === 0 ? <tr><td colSpan={5} className="table-empty">{notice || 'No company users returned from Phoenix Core.'}</td></tr> : filtered.map((user) => <tr key={user.id}><td><div className="person-cell"><span className="person-avatar">{user.display_name.slice(0, 2).toUpperCase()}</span><strong>{user.display_name}</strong></div></td><td>{user.username}</td><td><span className={`access-pill ${user.membership_status.toLowerCase()}`}>{user.membership_status}</span></td><td>{user.user_status}</td><td><button className="row-action" onClick={() => setNotice(`Selected ${user.display_name}`)}>View</button></td></tr>)}
        </tbody></table>
      </div>
    </> : <div className="access-panel"><div className="placeholder-icon"><Icon name={tab === 'Roles' ? 'settings' : 'people'} size={22} /></div><div><span className="section-label">ACCESS MANAGEMENT</span><h3>{tab}</h3><p>{tab === 'Roles' ? 'Organisation-scoped roles and their permissions are owned by Phoenix Core. This workspace will manage the company-facing role experience without creating system roles.' : 'Membership state is resolved by Phoenix Core and remains tenant-scoped. Company administrators can manage company access only within their authorised boundary.'}</p></div></div>}
